document.addEventListener("DOMContentLoaded", function () {
    ...
    axios.get("http://localhost:8080/lectures")
        .then((response) => {
            ...
            lectures.forEach((lecture, index) => {
                ...
                lectureImg.src = "/img/피아노-오겸비.png";
                lectureImg.style.width = "100%";
                lectureImg.style.height = "100%";
                lectureImg.style.objectFit = "cover"; // 추가된 부분
                lectureImg.style.borderRadius = "10px"; // 박스 모양 유지
                ...
                cartBtnDiv.addEventListener("click", function () {
                    if (userId) {
                        const lectureName = lectureInfoDiv.querySelector(".lectureEnrollname").innerText;
                        const teacherName = lectureInfoDiv.querySelector(".teacherEnrollname").innerText;
                        const hobbyCheckbox = lectureInfoDiv.querySelector(".hobbyCheckbox");
                        const examCheckbox = lectureInfoDiv.querySelector(".examCheckbox");

                        if (hobbyCheckbox.checked) {
                            addToCart(userId, lecture.lectureId, lectureName, teacherName, "취미반", 160000);
                        }

                        if (examCheckbox.checked) {
                            addToCart(userId, lecture.lectureId, lectureName, teacherName, "입시반", 200000);
                        }
                    } else {
                        alert("로그인이 필요합니다.");
                    }
    ...
    function addToCart(userId, lectureId, lectureName, teacherName, type, price) {
        axios.post("http://localhost:8080/cart/add/" + userId + "/" + lectureId)
            .then((response) => {
